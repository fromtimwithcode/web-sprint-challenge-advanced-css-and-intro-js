...
let objectIndex = artists.findIndex((obj => obj.id === 8));

artists[objectIndex].name = 'Vincent van Gogh';
...
function get20s(array) {
  const bornAndDied20th = [];
  let years = [];

  for (let i in array) {
    years.push(array[i].years.split(' - '));
    if (years[i][0] >= 1900 && years[i][1] <= 2000) {
      bornAndDied20th.push(array[i].name);
    }
  }

  return bornAndDied20th;
}
